...
import {
  ...
  BITCOIN_WALLET_ADDRESSES_INTERNAL_LOAD_REQUEST,
  BITCOIN_WALLET_ADDRESSES_INTERNAL_LOAD_SUCCESS,
  BITCOIN_WALLET_ADDRESSES_INTERNAL_LOAD_FAILURE
} from '../../../../../../../src/actions/bitcoin/wallet/addresses/internal/load';
...
describe('BITCOIN_WALLET_ADDRESSES_INTERNAL_LOAD_REQUEST', () => {
  it('equals "BITCOIN_WALLET_ADDRESSES_INTERNAL_LOAD_REQUEST"', () => {
    expect(BITCOIN_WALLET_ADDRESSES_INTERNAL_LOAD_REQUEST).toBe('BITCOIN_WALLET_ADDRESSES_INTERNAL_LOAD_REQUEST');
  });
});

describe('BITCOIN_WALLET_ADDRESSES_INTERNAL_LOAD_SUCCESS', () => {
  it('equals "BITCOIN_WALLET_ADDRESSES_INTERNAL_LOAD_SUCCESS"', () => {
    expect(BITCOIN_WALLET_ADDRESSES_INTERNAL_LOAD_SUCCESS).toBe('BITCOIN_WALLET_ADDRESSES_INTERNAL_LOAD_SUCCESS');
  });
});

describe('BITCOIN_WALLET_ADDRESSES_INTERNAL_LOAD_FAILURE', () => {
  it('equals "BITCOIN_WALLET_ADDRESSES_INTERNAL_LOAD_FAILURE"', () => {
    expect(BITCOIN_WALLET_ADDRESSES_INTERNAL_LOAD_FAILURE).toBe('BITCOIN_WALLET_ADDRESSES_INTERNAL_LOAD_FAILURE');
  });
});
...
describe('load', () => {
  ...
  it('dispatches an action of type BITCOIN_WALLET_ADDRESSES_INTERNAL_LOAD_REQUEST', () => {
  ...
  it('gets the addresses from AsyncStorage', () => {
    ...
    return loadAddresses()(dispatchMock).then(() => {
      ...
      expect(AsyncStorage.getItem).toHaveBeenCalledWith(BITCOIN_ADDRESSES_INTERNAL_STORAGE_KEY);
    });
  });

  it('dispatches an action of type BITCOIN_WALLET_ADDRESSES_INTERNAL_LOAD_SUCCESS with the addresses', () => {
    expect.hasAssertions();
  ...
  describe('when the function fails', () => {
    ...
    it('dispatches an action of type BITCOIN_WALLET_ADDRESSES_INTERNAL_LOAD_FAILURE with the error', () => {
      expect.hasAssertions();
